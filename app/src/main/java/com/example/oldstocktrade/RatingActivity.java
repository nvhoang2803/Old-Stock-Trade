    ...
    void initialize() {
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                Glide.with(RatingActivity.this).load(product.getImageURL().get(0)).into(product_image);
            }
        ...
        btn_done.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (rate == -1)
                    Toast.makeText(RatingActivity.this,"You haven't done yet!",Toast.LENGTH_SHORT).show();
                else {
                    DatabaseReference new_feedback = FirebaseDatabase.getInstance().getReference("Users").child(userID).child("Ratings").push();
                    HashMap<String,Object> hashMap = new HashMap<>();
                    hashMap.put("id",new_feedback.getKey());
                    hashMap.put("rating",rate);
                    hashMap.put("feedback",txt_feedback.getText().toString());
                    new_feedback.updateChildren(hashMap);
                    FirebaseDatabase.getInstance().getReference().child("Feedback").child(firebaseUser.getUid()).child(feedbackID).removeValue();
                    Toast.makeText(RatingActivity.this,"Feedback successfully",Toast.LENGTH_SHORT).show();
                    finish();
                }
            }
        });
